...
export function BanUserModal({ isOpen, user, onClose, onConfirm }: BanUserModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!banReason.trim()) {
      setError('Vui lòng nhập lý do cấm tài khoản.');
      return;
    }
    ...
    try {
      ...
      setError(err?.message || 'Có lỗi xảy ra khi thực hiện cấm tài khoản.');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl space-y-4 animate-in zoom-in-95 duration-200">
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-900 leading-snug">Cấm Tài Khoản Người Dùng</h3>
              <p className="text-xs text-slate-500">Áp dụng hình phạt cấm truy cập hệ thống</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-1.5">Thời Hạn Cấm</label>
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-1.5">
              Lý Do Cấm <span className="text-rose-500">*</span>
            </label>
            <textarea
              rows={3}
              value={banReason}
              onChange={(e) => setBanReason(e.target.value)}
              placeholder="Nhập chi tiết lý do vi phạm (ví dụ: Sử dụng phần mềm gian lận trong giải đấu)..."
