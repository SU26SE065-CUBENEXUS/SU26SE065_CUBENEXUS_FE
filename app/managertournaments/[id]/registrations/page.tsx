          ...
          <div className="flex items-center gap-3 mt-4 mb-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full rounded-xl border border-border bg-muted/10 pl-9 pr-3 py-2 text-xs text-foreground placeholder-muted-foreground outline-none focus:border-primary transition"
              />
            </div>
            <button
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        {[
          { label: 'Total Events', value: tournament.events.length, accent: 'text-foreground' },
          { label: 'Click Event Below', value: 'to view competitors', accent: 'text-primary text-sm font-semibold' },
          { label: 'Seed Override', value: 'via Events page', accent: 'text-muted-foreground text-sm' },
        ].map((s) => (
          <div key={s.label} className="rounded-xl bg-card border border-border px-5 py-4 shadow-sm">
            <p className="text-xs text-muted-foreground font-semibold uppercase">{s.label}</p>
            <p className={`font-black mt-1 ${s.accent}`}>{s.value}</p>
          </div>
        ))}
      </div>
