          ...
          <div className="space-y-4 max-w-4xl mx-auto w-full">
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 rounded-xl border border-slate-200 bg-white shadow-2xs text-slate-900">
                ...
                <button
                  disabled={isLoading || Number(roundNumber) !== 1 || isScramblesReady}
                  onClick={() => setIsGenerateGroupsOpen(true)}
                  className={`sm:self-center inline-flex items-center justify-center gap-1.5 rounded-lg px-3.5 py-2 text-xs font-semibold transition shadow-2xs ${isScramblesReady
                    ? 'bg-slate-100 border border-slate-200 text-slate-400 cursor-not-allowed'
                    : 'bg-indigo-600 text-white hover:bg-indigo-700 disabled:opacity-60 cursor-pointer'
                    }`}
                  title={Number(roundNumber) !== 1 ? 'Round 2+ groups must be created through Advance Round.' : isScramblesReady ? 'Cannot regenerate groups after scrambles have been generated.' : 'Generate Round 1 groups'}
                >
                  <Shuffle className="h-3.5 w-3.5" />
                  {Number(roundNumber) !== 1 ? 'Use Advance Round' : isScramblesReady ? 'Groups Locked (Scrambles Ready)' : groupsExist ? 'Regenerate Groups' : 'Configure & Generate'}
                </button>
              </div>
