...
export default function A01VideoReviewPage() {
  ...
  useEffect(() => {
    getAdminTournaments({ page: 1, pageSize: 100 })
      .then((result) => {
  ...
  return (
    <main className="mx-auto w-full max-w-7xl space-y-6 p-6">
      <header><h1 className="text-2xl font-black text-slate-900">A01 Video Review</h1><p className="mt-1 text-sm text-slate-500">Chỉ hiện attempt hợp lệ đã hoàn thành và có video evidence.</p></header>
      ...
      {loading ? <Loader2 className="h-6 w-6 animate-spin text-indigo-600" /> : (
        <div className="grid gap-5 lg:grid-cols-[1fr_1.3fr]">
          <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
            ...
            {attempts.length === 0 ? <p className="p-5 text-sm text-slate-500">Chưa có video A01 hợp lệ chờ review.</p> : attempts.map((item) => (
              <article key={item.attemptId} className="flex items-center justify-between gap-3 border-b border-slate-100 p-4 last:border-0">
                ...
                <button onClick={() => void openVideo(item.attemptId)} disabled={busy === item.attemptId} className="inline-flex items-center gap-1 rounded-lg bg-indigo-600 px-3 py-2 text-xs font-bold text-white disabled:opacity-50"><PlayCircle className="h-4 w-4" /> Xem video</button>
              </article>
            ))}
          </section>
